...
export function BudgetImpactPill({ result, className, size = "xs" }: Props) {
  ...
  // > 5% turns RED ("over-leveraged / unaffordable" — Gate 8 hard cap).
  // 2%–5% is amber (above the MaxRisk target but still tradeable).
  const tone = pct > AFFORDABILITY_CAP_PCT
  ...
  const tooltip = b.overBudget
    ...
    : pct > AFFORDABILITY_WARN_PCT
      ? `Over MaxRisk: this trade uses ${pct.toFixed(1)}% of your $${b.accountBalance.toLocaleString()} account ($${b.contractCost.toFixed(0)}). Above the ${AFFORDABILITY_WARN_PCT}% target; hard cap is ${AFFORDABILITY_CAP_PCT}%.`
