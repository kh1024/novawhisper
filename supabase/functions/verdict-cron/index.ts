...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: configs, error: cfgErr } = await admin
      ...
      .select("owner_key, webhook_url, alert_on_wait")
      .eq("enabled", true)
    ...
    for (const cfg of configs ?? []) {
      ...
      try {
        ...
        // 3. Get verdicts via existing edge function.
        const verdictResp = await fetch(`${SUPABASE_URL}/functions/v1/portfolio-verdict`, {
          ...
          body: JSON.stringify({
            positions: positions.map((p) => ({
              ...
              strike: Number(p.strike), strikeShort: p.strike_short != null ? Number(p.strike_short) : null,
        ...
        await admin.from("verdict_cron_config").update({
          ...
          last_run_status: `ok · ${positionCount} pos · ${fired} fired`,
        }).eq("owner_key", ownerKey);
